  ...
  const handleAddOrEditTeacher = async (e: React.FormEvent) => {
    ...
    if (editingTeacher) {
      ...
      try {
        ...
        toast.error(err.message || 'خطا در افزودن معلم');
      }
    }
  };
    ...
    <Card>
      <CardHeader>
          ...
          <div className="flex gap-2">
            ...
            <Dialog open={open} onOpenChange={(isOpen) => { setOpen(isOpen); if (!isOpen) resetForm(); }}>
              <DialogTrigger asChild>
                <Button className="gap-2"><Plus className="w-4 h-4" />افزودن معلم</Button>
              </DialogTrigger>
              <DialogContent dir="rtl">
                ...
                <form onSubmit={handleAddOrEditTeacher} className="space-y-4 pt-4">
                  ...
                  {!editingTeacher && (
                    <div className="space-y-2"><Label>رمز عبور</Label><Input type="password" value={password} onChange={(e) => setPassword(e.target.value)} required minLength={6} /></div>
                  )}
        ...
        <div className="relative mt-4">
          ...
          <Input placeholder="جستجوی نام معلم..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="pl-10" dir="rtl" />
        </div>
      </CardHeader>
                    ...
                    <div className="flex gap-2">
                      ...
                      <AlertDialog>
                        ...
                        <AlertDialogContent dir="rtl">
                          <AlertDialogHeader>
                            <AlertDialogTitle>آیا از حذف این معلم مطمئن هستید؟</AlertDialogTitle>
                            <AlertDialogDescription>این عمل قابل بازگشت نیست و پروفایل کاربر را نیز حذف می‌کند.</AlertDialogDescription>
                          </AlertDialogHeader>
                          <AlertDialogFooter>
                            <AlertDialogCancel>انصراف</AlertDialogCancel>
                            <AlertDialogAction onClick={() => handleDeleteTeacher(teacher)}>حذف</AlertDialogAction>
                          </AlertDialogFooter>
                        </AlertDialogContent>
                      </AlertDialog>
                    </div>
