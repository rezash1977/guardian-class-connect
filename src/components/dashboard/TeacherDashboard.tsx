...
import { cn } from "@/lib/utils"; // <-- FIX: Added this import
...
const TeacherDashboard = () => {
  ...
  useEffect(() => {
    if (selectedClassSubjectId && date && lessonPeriod) {
      fetchAttendance();
    } else {
      setAttendanceStatus({});
    }
  ...
  const fetchClassStudents = async (classId: string) => {
    const { data } = await supabase.from('students').select('id, full_name').eq('class_id', classId);
    setStudents(data || []);
  };
  ...
  const fetchAttendance = async () => {
    ...
    const statusMap = data?.reduce((acc, record) => {
  ...
  const handleAttendanceSubmit = async () => {
    if (!selectedClassSubjectId || !date || !lessonPeriod || Object.keys(attendanceStatus).length === 0) {
      toast.error('لطفاً کلاس، تاریخ، ساعت درسی و وضعیت دانش‌آموزان را مشخص کنید.');
      return;
    }
    setIsSubmitting(true);
    const formattedDate = format(date, 'yyyy-MM-dd');

    // Delete existing records for this specific class/date/period
    ...
    if (error) toast.error('خطا در ثبت حضور و غیاب: ' + error.message);
    else toast.success('حضور و غیاب با موفقیت ثبت شد.');
    setIsSubmitting(false);
  };
  ...
  const openDisciplineDialog = (record: DisciplineRecord | null) => {
    ...
    if (record) {
      setSelectedStudentId(students.find(s => s.full_name === record.students.full_name)?.id);
      ...
      <main className="container mx-auto px-4 py-8 space-y-6" dir="rtl">
        <Card>
          ...
          <CardContent className="grid grid-cols-1 md:grid-cols-3 gap-4">
            <Select value={selectedClassSubjectId} onValueChange={setSelectedClassSubjectId}>
              <SelectTrigger disabled={loading}><SelectValue placeholder="انتخاب کلاس..." /></SelectTrigger>
          ...
          <Card>
            <CardHeader>
              <div className="flex items-center justify-between">
                <div><CardTitle>ثبت حضور و غیاب</CardTitle><CardDescription>وضعیت هر دانش‌آموز را مشخص کنید</CardDescription></div>
                <AlertDialog>
                  <AlertDialogTrigger asChild><Button disabled={isSubmitting}>{isSubmitting && <Loader2 className="ml-2 h-4 w-4 animate-spin"/>}ثبت حضور و غیاب</Button></AlertDialogTrigger>
                  <AlertDialogContent dir="rtl">
                    <AlertDialogHeader><AlertDialogTitle>آیا مطمئن هستید؟</AlertDialogTitle><AlertDialogDescription>با این کار، سوابق قبلی حضور و غیاب برای این کلاس، تاریخ و ساعت درسی بازنویسی خواهد شد.</AlertDialogDescription></AlertDialogHeader>
        ...
        {selectedClass && (
          <Card>
              <CardHeader>
                <div className="flex items-center justify-between">
                    <div><CardTitle>موارد انضباطی</CardTitle><CardDescription>موارد انضباطی ثبت شده برای این کلاس</CardDescription></div>
                    <Dialog open={disciplineOpen} onOpenChange={(isOpen) => { setDisciplineOpen(isOpen); if (!isOpen) resetDisciplineForm(); }}>
                      ...
                      <DialogContent dir="rtl">
                        ...
                        <form onSubmit={handleDisciplineSubmit} className="space-y-4 pt-4">
                          ...
                          <Button type="submit" className="w-full">{editingDiscipline ? "ویرایش" : "ثبت"}</Button>
                        </form>
                      </DialogContent>
                    </Dialog>
                </div>
                  ...
                  <Table>
                      ...
                      <TableBody>
                          ...
                          disciplineRecords.map(rec => (
                              <TableRow key={rec.id}>
                                  <TableCell>{rec.students.full_name}</TableCell>
                                  ...
                                  <TableCell>{format(new Date(rec.created_at), 'yyyy/MM/dd')}</TableCell>
